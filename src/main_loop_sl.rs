    ...
    W: Word + HasBitWord<BitWord: DeserializeOwned> + DeserializeOwned,
{
    ...
    fn try_each_matching_postfix<'b, T>(
        ei: EnumerationInfo<W>,
        bm: &BackwardMap<W>,
        postfix_length: usize,
        should_cancel: ShouldCancel,
        backward_graph: &'b mut BackwardGraph<W>,
        counter_examples: &CounterExamplesCell<W>,
        inputs: &[State<W>],
    ) -> ControlFlow<Cancelled, Option<BackwardGraphPath<'b, W>>> {
        // TODO: Should this be called in a loop with a backwards-graph-path?
        Self::update_backward_graph(
            ei,
            bm,
            backward_graph,
